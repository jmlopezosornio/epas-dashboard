...
document.getElementById('excelFile').addEventListener('change', async e=>{ const f=e.target.files[0]; if(!f)return; try{state.collection=parseCollection(await f.arrayBuffer());populateCollectionFilters();renderCollection();document.getElementById('fileStatus').textContent=`Archivo local: ${f.name}`;updateTimestamp(document.getElementById('lastUpdate'));}catch(err){alert(err.message);} });
...
function parseWorks(buffer){
  ...
  for(let r=hi+1;r<matrix.length;r++){
    ...
    out.push({expediente:exp, obra:name, localidad:cleanText(matrix[r][l])||'Sin localidad', apertura:f>=0?excelDateToJS(matrix[r][f]):null, presupuesto:numeric(matrix[r][p]), adjudicado:a>=0?cleanText(matrix[r][a]):''});
  }
  return out;
}
...
function renderWorks(){
  ...
  body.innerHTML=sorted.length?sorted.map(w=>{const st=workStatus(w);return `<tr><td class="exp-cell">${escapeHtml(w.expediente)}</td><td class="work-name">${escapeHtml(w.obra)}</td><td>${escapeHtml(w.localidad)}</td><td>${fmtDate(w.apertura)}</td><td class="numeric">${fmtMoney(w.presupuesto)}</td><td><span class="status-pill ${st.cls}">${escapeHtml(st.label)}</span></td><td>${w.adjudicado?escapeHtml(w.adjudicado):'—'}</td></tr>`;}).join(''):'<tr><td class="empty-row" colspan="7">Sin obras para los filtros seleccionados.</td></tr>';
}
...
document.getElementById('worksFile').addEventListener('change',async e=>{const f=e.target.files[0];if(!f)return;try{state.works=parseWorks(await f.arrayBuffer());populateWorksFilters();renderWorks();document.getElementById('worksFileStatus').textContent=`Archivo local: ${f.name}`;document.getElementById('worksBadge').textContent=state.works.length;updateTimestamp(document.getElementById('worksLastUpdate'));}catch(err){alert(err.message);}});
